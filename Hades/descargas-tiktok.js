...
try {
let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: texto, contextInfo: { externalAdReply: { title: '𝚃𝙷𝙴 CHICA 𝙱𝙾𝚃', body: null, thumbnail: imagen1, sourceUrl: 'https://github.com/-----------' }, mentionedJid: [m.sender] }}}, { quoted: m })
...
await conn.relayMessage(m.chat, prep.message, { messageId: prep.key.id, mentions: [m.sender] })
let buttons = [{ buttonText: { displayText: '𝙰𝚄𝙳𝙸𝙾' }, buttonId: `${usedPrefix}mp3` }]
conn.sendMessage(m.chat, { video: { url: url}, caption: '𝚃𝙷𝙴 CHICA 𝙱𝙾𝚃', footer: await shortUrl(url), buttons }, { quoted: m })
} catch {
